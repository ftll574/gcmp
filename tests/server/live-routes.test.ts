  ...
  destinations: [
    {
      ...
      airlines: [
        { airline_code: 'BR', airline: 'EVA Air', schedule: [{ day: 'Mon', times: [] }, { day: 'Mon', times: [] }], seasonal_note: null, service_type: 'scheduled' },
...
describe('live route gateway', () => {
  test('normalizes only active scheduled passenger listings without claiming an operator', () => {
    ...
    expect(result.routes[0]?.carriers).toEqual([{ code: 'BR', name: 'EVA Air', days: ['Mon'], seasonalNote: null }]);
