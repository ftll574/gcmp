...
- **Convergence-contract execution** (`docs/convergence-contract.md`, 2026-08-26 three-round scope grill) — north star frozen to *bounded rigor serving the BR/CX end-to-end acceptance line*; four divergent surfaces deleted outright:
  - **Earning/PQM/RDM secondary panel** — `EarningPanel`, `ProgramPicker`, `Glossary`, tier selector, all-programs "where to credit" view, the per-program ¢/mile valuations loader/schema/data (`public/data/valuations/**`); the pure earning engine stays under `src/lib/calc/**` for the plain-text share post
  - **Map extras** — projection picker (URLs still render the projection they carry; otherwise the default), bearing labels, PNG/SVG download menu (`src/lib/svg-to-png.ts`)
  - **Award fee-schedule cards** — display-only era-pinned fees carried drift obligations without feeding any total; `AwardFeeScheduleSchema`/`getAwardFees()` and the CX fees data block removed
  - **zh-CN/ja locales** — en + zh-TW remain; zh-Hans navigator variants now fall back to zh-TW (Taiwan-first)
  - Legacy URL parameters `p/c/st/fc/proj` still parse for backward compatibility with already-shared URLs but are inert in the UI; cabin selection moved into the planning-context block because it drives award pricing

### Added

- **UI re-aligned to DESIGN.md + first-party dark mode** (`docs/convergence-contract.md`, user-approved "重新全部設計") — the v1.4 Apple-HIG token layer had silently diverged from the design system (system-ui fonts, iOS blue on white, pill radii, card shadows — several AI-slop-blacklist violations). The token layer is rewritten to the editorial parchment/charcoal/deep-teal system: IBM Plex Sans/Mono explicit, canonical type scale (brand 32 / totals 28 mono hero / body 16 / chip 14 / detail 12), small intentional radii (chips 4px, buttons 6px), hairline-only separation with shadows restricted to detached layers, and a designed "night chart" dark mode (charcoal slate paper, parchment ink, `#5FB9CC` teal) documented in DESIGN.md §Dark mode. Two ghost tokens that never existed (`--type-title-3`, `--radius-control`) are now defined, fixing silently-fallback styles. Pure presentation change — zero behavior/routing/data impact
- **Alliance-first two-step selection + known-nonstops explorer** (`docs/convergence-contract.md` §4 promotion, user-approved) — the planning context now leads with alliance chips (Star Alliance / oneworld / SkyTeam): clicking one jumps to that alliance's market-preferred product and shows its member carriers; a new `DestinationsPanel` under the route chain lists each member airline's KNOWN nonstop destinations sourced only from the flight-schedule catalog (`destinationsForCarrier()` in `src/lib/rtw/route-discovery.ts`, TPE-first origins, suspended rows dropped, chart-verified outranking community-corrected). Clicking a chip extends the active route chain; chips that cannot legally attach are disabled with a hint naming the required origin; carriers without catalog rows (CX today) show an explicit 「班表未收錄」 note instead of implying anything about their network. Alliance is derived state — no URL-schema change, no new data subsystem; CX/partner network harvesting stays deferred as the follow-up research round
- **Violation fix hints v1** (`docs/convergence-contract.md` §3) — pure `src/lib/rtw/fix-hints.ts` maps FAIL findings of exactly five contracted high-frequency rules to ONE textual remedy each: segment over/under-count, stopover over/under-count (convert/promote counts computed), distance-cap overage when raw numbers are available (honest generic remedy otherwise), per-leg carrier switch naming affected legs 1-based, and the three ocean-crossing constructions (missing Pacific / Atlantic / both). Every other rule falls back to plain rule text. Rendered as a secondary ↪ remedy line under fail findings on the validation panel, localized en/zh-TW. No auto-generated alternative routes — explicitly out of scope until v1 survives contact with users
- **Corrupted share-URL regression tests** (`tests/components/App.routing-error.test.tsx`) — full-`<App>` integration tests with a stubbed fetch serving the real `public/data` tree: a bad hash (`YVRHKG` path token / malformed `d=` date) must surface its typed parse error as the ⚠ banner AND the banner must survive mount; a healthy hash still loads legs with no banner
...
- **True open-jaw distance modeling** — per-product `openJawDistancePolicy` (default `excluded-from-distance`) gates whether inter-group open-jaw gaps contribute great-circle distance to the priced total; the CX oneworld Multi-carrier Award opts in (`counts-toward-distance`) per community agent practice (FlyerTalk 2184572), while jaws stay invisible to segment, ocean-crossing, and direction checks; decision record `docs/decisions/open-jaw-distance.md`
- **Award-zone price breakdown** — the RTW validation panel now shows the matched award zone's distance range plus every cabin price the catalog knows for that zone (`quoteAwardZone()` in `src/lib/rtw/award-pricing.ts`, rendered by the `AwardZoneBreakdown` component); cabins absent from a chart render as an em-dash instead of a guess (the ANA archived chart prices business only), localized in all 4 locales
- **Award fee-schedule model** — `AwardFeeScheduleSchema` + `getAwardFees()` accessor, CX seed fees era-pinned 2018-05 (date-change HKD40+4000mi/sector, reissue HKD100+10000mi, refund HKD120+12000mi, community-corrected PTT source), validation-panel fee card + i18n ×4 (commits `7b3dd51`, `6a68f09`)
- **Qantas Classic Flight Reward pricing entry** — partial-cabin distance band {19,201–35,000 mi ⇒ 318,000 business} chart-verified against the pre-Aug-2025 chart era via PointHacks thread 24765; smaller brackets/economy intentionally absent pending verification (post-2025-08-05 revision totals recorded as rules-version drift, not merged); pricing coverage now 4 of 7 catalog products
- **China Airlines SkyTeam partner award zone-pair pricing entry** — the Era-2 chart recovered in §A8 (`docs/calibration-set.md`) lands as a full 66-cell `zoneMatrix` over the 11 published zones (NEA…SWP) with Economy / Premium Economy / Business prices, `published-chart` confidence, `asOfEra` 2025-10; schema grows `pricingModel: 'zone-pair'` (`AwardPricingZonePairProductSchema`), a shared `AwardCabinPricesSchema` (optional positive-int cabins + ≥1-priced refine), and `'premium-economy'` joins `AwardPricingCabinSchema`; new engine accessor `getZonePairQuote()` (`src/lib/rtw/award-pricing.ts`) matches upper-triangle cells in either direction and returns null for unpriced cabins (Era-2 discontinued First Class) instead of guessing; zone-pair products return null from the distance-band path so existing band/fixed contracts stay untouched; six `calib.ci-skyteampartner.*` Iron-Rule pins extend the calibration suite to 24 active structural tests; planner coverage intentionally remains 4 of 7 until airport→region wiring surfaces itinerary-level CI estimates
- **Chart-drift review checklist** — `docs/process/chart-drift-checklist.md` codifies the quarterly `rv`-era chart re-verification workflow distilled from Phase 2–9 research passes: Wayback CDX prefix queries (the `filter=` parameter soft-fails to empty-200s — always run a no-filter control), legacy 13–15 KB title-only vs Next.js RSC shell discrimination, negative captures graded as evidence, the dual confidence vocabulary (product ladder `official-fixed`/`published-chart`/`reference-recheck` vs research ladder `chart-verified`/`community-corrected`, kept separate by design because a merge would ripple into UI pill classes and schema enums), CONFLICT #N ledger conventions (side-by-side readings, never resolved silently), negatives logged to prevent re-digging, and era-bracket pinning tied to the URL's `rv=YYYY.Q`
- **OWE / Star Alliance RTW Fare cash-fare adjudication** — §A8(b) confirms both products are paid fares sold in money (oneworld Explorer continent-based, Star Alliance RTW distance-based; miles flow earn-side only), so they stay out of award-pricing scope by design; documented in README Current Limits
- **Flight-schedule model & calendar** — per-leg departure dates land per decision record `docs/decisions/flight-schedule-model.md` (S1–S6): `Leg.departsOn` (ISO `YYYY-MM-DD`, absent = undated) shared through the URL `d=` parameter; pure engine rules `leg-chronology` (fail — consecutive dated legs may not run backwards), `trip-dates-mismatch` (warn — trip start/end vs first/last leg dates), and `schedule-day-mismatch` (warn — dated leg outside the carrier+pair's operating weekdays, never a fail); curated evidence-graded seed catalog `public/data/schedules/current.json` (10 directional entries — CI×6 `chart-verified` from the official 2025-Q1 timetable PDF recovered via Wayback, BR×2 + JX×2 `community-corrected` via AeroRoutes; expired CI windows stay silent by design); window-aware custom calendar popover disables non-operating days only for catalog-covered pairs while 「班表未知」 pairs keep free input, with a frequency chip and 「跳到下一個可用日」 quick-fix
- **National-carrier schedule coverage drive** — schedule catalog grows 10 → **138 directional entries, all `chart-verified`**: STARLUX ×78 harvested from the airline's own public schedule API (`ecapi.starlux-airlines.com/flightSchedule/v2/timetable` returns per-flight operating-date arrays; every route × both directions; repeatable harvester `scripts/harvest-jx-schedules.mjs` with adaptive 99210-throttle backoff and resume-from-valid-cache joins the quarterly chart-drift cadence), EVA ×28 from AeroRoutes official filings (current NS26/W26 seasons incl. the TPE→IAD launch grids and the TPE→DEL Dec-2026 launch window), China Airlines ×32 from a complete transcription of the official Issue-2 timetable PDF (2025-Q1 window stays self-silencing; a newer edition exists but was never archived and the live page blocks crawlers — recorded as an honest gap); same-pair flight-group rows collapse into one entry with day-split notes, sequential season windows map onto the MM-DD season fields; decision-record addendum documents the API discovery and derivation rulings (`docs/decisions/flight-schedule-model.md`); research appendix §A11 adds methodology, verbatim quotes, an 11-item negatives ledger, and fetch budgets
- **CI zone-pair award price display** — the Phase-9 deferral closes: a curated 65-station chart-verified station→zone map (`public/data/geo/ci-zones.json`, zod-validated `CiZoneMapSchema` with duplicate-airport rejection) plus pure resolvers `resolveCiZone()`/`quoteCiLeg()` (reusing `getZonePairQuote()`, never reimplementing matrix math) let the validation panel render **per-leg CI SkyTeam partner-award quotes** whenever both endpoints resolve; unmapped endpoints show 「區域未知」, surface sectors are labeled unpriced, an unpriced cabin on a mapped pair is distinguished from an unknown zone, and the 合計 line is explicitly captioned as a segment-sum lower bound because CI's whole-ticket arithmetic remains officially unverified (rulings Z1–Z5 in `docs/decisions/ci-zone-resolution.md`)

### Fixed

- **Right-panel tab/cabin active state invisible in dark mode** — `.inspector-tab.active`/`.cabin-tab.selected` used `background: var(--system-background)` + a hairline `box-shadow`, which on the new charcoal paper made the active tab indistinguishable from inactive ones (users couldn't tell which tab they were on). Both now use the accent-underline vocabulary `color: var(--tint); box-shadow: inset 0 -2px 0 var(--tint)`; `.group-tab.active` ring unchanged (user-reported 「網頁右半邊幾乎無法順暢使用」)
- **Cross-alliance carriers leaked into the product picker** — the product `<select>` in `RtwPlanningContext` listed every product regardless of the active alliance, so after picking Star Alliance users still saw oneworld/SkyTeam products (「我選完聯盟之後，航空公司怎麼還有其他聯盟的」). The select now narrows to `products.filter(p => p.alliance === selectedProduct.alliance)` (alliance-less products only when no alliance is active); cross-alliance products stay reachable through the alliance chips. Pinned by `tests/components/RtwPlanningContext.test.tsx`
- **Silent failure on corrupted share URLs** — App's default-product effect called `setRouting` on mount whenever `rtwProductId` was undefined (exactly the post-parse-failure state), wiping `routing.error` before the ⚠ banner could render; users saw a blank planner instead of "Invalid IATA code in chain". The effect now defers while `routingError` is set and resumes on the first real user edit (found via live computer-use walk-through; pinned by the new integration tests)
- **Dev server killed by atomic-save temp files** — chokidar's Windows watcher exits with EBUSY when an editor's `<name>.<uuid>.tmpdir/<file>.tmp` transient disappears mid-watch, crashing `vite` on every externally-driven file save; `server.watch.ignored` now excludes those patterns

### Changed

- **DestinationsPanel grouped four-tier (continent → subregion → country → airport)** — the known-nonstops explorer now folds destinations into a hairline tree instead of a flat chip row (user-reported 「下面的機場也要分層可以分區塊,例如洲->區塊->國家->機場」). Pure `groupDestinationsByGeo()` in `src/lib/rtw/route-discovery.ts` sorts continents by a fixed `CONTINENT_ORDER` (`asia` first), orders `null` subregions last within a continent, then sorts countries alphabetically; unknown airports land in an explicit `??`/`unmapped` bucket instead of vanishing. `public/data/geo/current.json` gains an optional `subregion` field (19 rows: northeast/southeast/south Asia; west/central Europe). Falls back to the flat row when geo maps are absent. Pinned by `tests/lib/rtw/route-discovery.test.ts` + `tests/components/DestinationsPanel.test.tsx`
...
- **Calibration suite extended to 18 active tests, zero open engine-gap TODOs** — QF segment cap + carrier minimum and ANA archived band pricing + surface exclusion are joined by CX any-First-sector pricing (whole-ticket-at-First pinned through live bands, boundary repricing 155k→190k across the 14,000-mile edge via a test-local rv=2018.Q2 official-chart fixture transcribed from Wayback capture 20180528013013, premium economy — absent from that chart — pricing as null), BR TPE–GUM network-gap warnings, and the activated `calib.cx-multicarrier.open-jaw-distance-counts` pin (the BKK⇢SIN inter-group jaw's great-circle distance counts into the CX oneworld Multi-carrier priced total, exact-delta asserted per FlyerTalk 2184572)
...
- **zh-CN/ja award-panel strings translated** — the whole `rtw.award` block (estimate, miles/band suffixes, sources, confidence labels, cabin names) previously rendered in English in both locales; now natively translated alongside the new zone label
- **zh-CN/ja locale parity completed** — the entire residual English `rtw.*` block (~90 keys per locale: noProducts/planning/validation headers, productKind, startEnd 缺口程/オープンジョー, limits, status badges, and the rest) is natively translated in both locales; `index.html` title/meta now describe the RTW planner ("gcmp — Round-the-World Award Route Planner") instead of the pre-pivot earning calculator
- **Cathay multi-carrier bands rebased to the pinned fourth-era chart** — the FT 2184572 Jan-2025 mystery (230,000 miles at a self-stated 19,442 flown miles, matching no frozen-era cell) closes at cell level: fourth-era Zone 10 (18,001–20,000 mi) Business = 230,000 Asia Miles, pinned by two independent data points (Prince of Travel 2025-07-16; Suitesmile post-devaluation grid transcription 2026-05-02) with a third browser-render cross-check (§A9); zone edges are byte-identical to rv=2018.Q3 while prices rise across the board (z09 Business 155k→210k, z10 165k→230k, z11 185k→250k); the exact effective date stays an open bracket (2023-02-26, 2025-01-25], and the official flights.cathaypacific.com page remains DNS-unresolvable from this network — both recorded as honest gaps
- Fix: clarified RTW workbench layout (`a5ec1bf`)
- **Share URLs carry per-leg departure dates** — optional `d=` query parameter mirrors `op`'s shape (`;` groups / `,` legs / empty cell = undated); when present each group's date count must equal its leg count (typed parse error otherwise); URLs without `d=` parse unchanged, so existing shares stay valid
