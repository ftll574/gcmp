...
- **DATA_MANIFEST.json:** every file under `public/data/` (206 datasets) is now cataloged with kind (input/derived/shard/meta), producing script, source, license, bytes and sha256 — generated by `scripts/build-data-manifest.ts`, verified by `scripts/verify-data-manifest.ts` (wired into CI) so the data layer never drifts from disk.
- **License annotations:** 47 new `*.meta.json` files beside previously unannotated datasets (alliances, award-pricing, rtw-products, rtw-seasonal, markets, geo, network-gaps, schedules, official-schedules, programs ×36) — sources mapped per THIRD_PARTY_NOTICES.md; unknown ones stay `unattributed (pending confirmation)`.
- **Orphan cleanup:** removed `public/data/world-countries-110m.json` (zero references; 50m is the only loaded map file). `flightsfrom-*` and legacy `mrairspace-flight-number-candidates.json` are inputs consumed by `build-flight-number-layer.ts` — kept and annotated.
- **Baseline sync:** test-count claims updated to 1185 tests / 106 files across README and CLAUDE.md.
