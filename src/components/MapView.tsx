 ...
 *
 * This avoids one SVG/DOM node per airport, which became the bottleneck once
 * airport picking was added.
 */
...
export function MapView({
  ...
  const routeLegs = useMemo<RouteLegDatum[]>(() => {
    return groups.flatMap((group, groupIndex) =>
      group.legs.flatMap((leg, index) => {
        const from = airportLookup.get(leg.from);
        const to = airportLookup.get(leg.to);
        if (!from || !to) return [];
        return [{ key: `${groupIndex}-${leg.from}-${leg.to}-${index}`, groupIndex, from, to }];
      }),
    );
  }, [groups, airportLookup]);
  ...
  return (
    <div className="map-view deck-map-view" style={{ width, height }}>
      ...
      {projection !== 'mercator' && (
        <div className="deck-map-projection-note">
          deck.gl spike renders Web Mercator for performance.
        </div>
      )}
